...
import Shift from '../model/ShopA1.schema.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(shift);
    req.app.get('io').emit('shopa1', shift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    const generatedCode = (maxCodeResult[0]?.maxCode || 29999) + 1;
    ...
    if (updatedShift.status === 'current') {
      req.app.get('io').emit('shopa1', updatedShift);
    }
...
router.post('/newshift', async (req, res) => {
  try {
    ...
    res.json(newShift);
    req.app.get('io').emit('shopa1', newShift);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (lastShift) {
      ...
      await lastShift.save();
      req.app.get('io').emit('shopa1', lastShift);
    }
