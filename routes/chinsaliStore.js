...
import Store from '../model/ChinsaliStore.schema.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(store);
    req.app.get('io').emit('chinsalistore', store);
  } catch (error) {
...
router.post('/newshift', async (req, res) => {
  try {
    ...
    req.app.get('io').emit('chinsalistore', newShift);
  } catch (error) {
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    req.app.get('io').emit('chinsalistore', updatedShift);
  } catch (error) {
    res.status(500).json({ message: 'Internal Server Error' });
  }
});
