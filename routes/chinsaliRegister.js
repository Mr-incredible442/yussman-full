...
import Register from '../model/ChinsaliRegister.schema.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(register);
    req.app.get('io').emit('chinsaliregister', register);
  } catch (error) {
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    req.app.get('io').emit('chinsaliregister', updatedShift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.post('/newshift', async (req, res) => {
  try {
    ...
    res.json(newShift);
    req.app.get('io').emit('chinsaliregister', newShift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.json(lastShift);
    req.app.get('io').emit('chinsaliregister', lastShift);
  } catch (error) {
