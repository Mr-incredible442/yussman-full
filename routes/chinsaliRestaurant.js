...
import Shift from '../model/Chinsali.schema.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(shift);
    req.app.get('io').emit('chinsalirestaurant', shift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    if (updatedShift.status === 'current') {
      req.app.get('io').emit('chinsalirestaurant', updatedShift);
    }
...
router.post('/newshift', async (req, res) => {
  try {
    ...
    res.json(newShift);
    req.app.get('io').emit('chinsalirestaurant', newShift);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (lastShift) {
      ...
      await lastShift.save();
      req.app.get('io').emit('chinsalirestaurant', lastShift);
    }
