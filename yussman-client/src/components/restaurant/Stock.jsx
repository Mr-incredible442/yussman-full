...
import NewProductModal from '../restaurant/modals/NewProductModal';
...
function Stock() {
  ...
  const { shift } = useContext(RestaurantContext);
  ...
  const handleSaveEdit = (e, itemId) => {
    ...
    axios
      .post(
        `${RESTAURANT_LOCAL_URL}/${shift._id}/editstock/${itemId}`,
        modalData,
      )
      .then(() => {
        setIsLoading(false);
        ...
        const allItems = [...drinks, ...food, ...bakery];
