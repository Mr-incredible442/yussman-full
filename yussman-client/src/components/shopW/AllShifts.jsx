...
function AllShifts() {
  ...
  const groupShiftsByMonth = () => {
    ...
    const sortedMonths = Object.keys(groupedShifts).sort((a, b) => {
      ...
      const firstDateA = groupedShifts[a][0]?.dateIn;
      const firstDateB = groupedShifts[b][0]?.dateIn;
    ...
    sortedMonths.forEach((month) => {
      groupedShifts[month].sort((a, b) => {
        const dateA = a.dateIn ? new Date(a.dateIn).getTime() : null;
        const dateB = b.dateIn ? new Date(b.dateIn).getTime() : null;
