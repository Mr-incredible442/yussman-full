...
import { BASE_URL, SHOPA1_URL } from '../helpers/variables';

export const ShopA1Context = createContext();
const ShopA1Reducer = (state, action) => {
  switch (action.type) {
    case 'SET_SHIFT':
      return {
...
export const ShopA1ContextProvider = ({ children }) => {
  const [shift, dispatch] = useReducer(ShopA1Reducer, { shift: null });
  const [updatedStock, setUpdatedStock] = useState({});

  useEffect(() => {
    apiCall.get(SHOPA1_URL).then((response) => {
  ...
  return (
    <ShopA1Context.Provider value={{ shopA1Shift: updatedStock, dispatch }}>
      {children}
    </ShopA1Context.Provider>
  );
};
