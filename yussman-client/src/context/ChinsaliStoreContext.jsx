...
import { CHINSALI_STORE_URL, BASE_URL } from '../helpers/variables';

export const ChinsaliStoreContext = createContext();

const chinsaliStoreReducer = (state, action) => {
  switch (action.type) {
    case 'SET_SHIFT':
      return {
...
export const ChinsaliStoreContextProvider = ({ children }) => {
  const [shift, dispatch] = useReducer(chinsaliStoreReducer, { shift: null });
  const [updatedStock, setUpdatedStock] = useState({});

  useEffect(() => {
    apiCall.get(CHINSALI_STORE_URL).then((response) => {
  ...
  return (
    <ChinsaliStoreContext.Provider
      value={{ storeShift: updatedStock, dispatch }}>
      {children}
    </ChinsaliStoreContext.Provider>
  );
};
